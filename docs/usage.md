...
`.spawn.toml` may set the default agent for a workspace. Host access still requires an explicit `--access ...` at launch time, and cross-workspace cache sharing an explicit `--cache shared`.
...
| `--cache <scope>` | Build cache scope: `workspace` (default, private to this workspace), `shared` (opt in; flag only) |
